...
// @namespace    https://github.com/badraxas/PlexDL
// @version      2.11.1
...
(function() {
    ...
    function selectServerAndNavigate(forceSelection = false) {
        ...
        getXml(resourcesUrl, function(resourcesXml) {
            ...
            // Si solo hay un servidor, usarlo directamente
            if (servers.length === 1) {
                console.log('PlexDL: Solo un servidor disponible, conectando automáticamente...');
                const server = servers[0];
                const libraryUrl = `https://plex-redirect-production.up.railway.app/library?accessToken=${encodeURIComponent(server.accessToken)}&baseURI=${encodeURIComponent(server.baseUri)}`;
            ...
            if (preferredServer) {
                ...
                const libraryUrl = `https://plex-redirect-production.up.railway.app/library?accessToken=${encodeURIComponent(preferredServer.accessToken)}&baseURI=${encodeURIComponent(preferredServer.baseUri)}`;
                window.open(libraryUrl, '_blank');
            } else {
    ...
    // --- Descarga abriendo el enlace en una nueva pestaña ---
    function tryDownload(url, filename, fileSize, extraData = {}) {
        // Extraer datos para la web intermedia
        const urlObj = new URL(url);
        const accessToken = urlObj.searchParams.get('X-Plex-Token') || '';
        const baseURI = urlObj.origin;
        const partKey = urlObj.pathname;
        // Montar la URL del redirector con todos los datos
        let redirector = 'https://plex-redirect-production.up.railway.app/?'
        ...
        // Añadir datos extra
        if (extraData.title) redirector += '&title=' + encodeURIComponent(extraData.title);
        if (extraData.episodeTitle) redirector += '&episodeTitle=' + encodeURIComponent(extraData.episodeTitle);
        if (extraData.seasonNumber) redirector += '&seasonNumber=' + encodeURIComponent(extraData.seasonNumber);
        if (extraData.episodeNumber) redirector += '&episodeNumber=' + encodeURIComponent(extraData.episodeNumber);
        if (extraData.posterUrl) redirector += '&posterUrl=' + encodeURIComponent(extraData.posterUrl);
        window.open(redirector, '_blank');
    }

    // --- Recursivo: descarga todos los episodios/archivos ---
    function downloadRecursive(xmlMeta, baseUri, accessToken) {
        // 1. Si hay directorios, recorrer recursivamente
        ...
        if (directories.length > 0) {
            for (let i = 0; i < directories.length; i++) {
                ...
                getXml(childrenUrl, function(xmlChildren) {
                    downloadRecursive(xmlChildren, baseUri, accessToken);
                });
            }
            return;
        }
        // 2. Si hay vídeos, descargar cada uno
        ...
        if (videos.length > 0) {
            for (let j = 0; j < videos.length; j++) {
                ...
                // Datos extra
                const title = video.getAttribute('grandparentTitle') || video.getAttribute('title') || '';
                const episodeTitle = video.getAttribute('title') || '';
                ...
                const art = video.getAttribute('art') || '';
                const posterUrl = thumb ? `${baseUri}${thumb}?X-Plex-Token=${accessToken}` : (art ? `${baseUri}${art}?X-Plex-Token=${accessToken}` : '');
                const downloadUrl = `${baseUri}${keyBase}${fileName}?download=0&X-Plex-Token=${accessToken}`;
                tryDownload(downloadUrl, fileName, fileSize, {
                    title,
                    episodeTitle,
                    seasonNumber,
                    episodeNumber,
        ...
        // 3. Caso simple: un solo archivo
        const singlePartKey = xmlMeta.evaluate(
            `//Media/Part[1]/@key`,
            xmlMeta, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
        ).singleNodeValue;
        const singlePartFile = xmlMeta.evaluate(
            `//Media/Part[1]/@file`,
            xmlMeta, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
        ).singleNodeValue;
        const singlePartSize = xmlMeta.evaluate(
            `//Media/Part[1]/@size`,
            xmlMeta, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null
        ).singleNodeValue;
        if (singlePartKey && singlePartFile) {
            const partKey = singlePartKey.textContent;
            const fileFull = singlePartFile.textContent;
            const fileSizeBytes = singlePartSize ? singlePartSize.textContent : '';
            const keyBase = partKey.replace(/\/[^\/]+$/, '/');
            const fileName = fileFull.split('/').pop();
            let fileSize = '';
            if (fileSizeBytes) {
            ...
            const title = video ? (video.getAttribute('grandparentTitle') || video.getAttribute('title') || '') : '';
            const episodeTitle = video ? (video.getAttribute('title') || '') : '';
            const seasonNumber = video ? (video.getAttribute('parentIndex') || '') : '';
            const episodeNumber = video ? (video.getAttribute('index') || '') : '';
            const thumb = video ? (video.getAttribute('thumb') || '') : '';
            const art = video ? (video.getAttribute('art') || '') : '';
            const posterUrl = thumb ? `${baseUri}${thumb}?X-Plex-Token=${accessToken}` : (art ? `${baseUri}${art}?X-Plex-Token=${accessToken}` : '');
            const downloadUrl = `${baseUri}${keyBase}${fileName}?download=0&X-Plex-Token=${accessToken}`;
            tryDownload(downloadUrl, fileName, fileSize, {
                title,
        ...
        alert('No se encontró ningún archivo para descargar.');
    }

    // --- Extrae baseUri, accessToken y redirige a modal apropiado ---
    function getMetadata(resourcesXml) {
        ...
        getXml(libraryUrl, function(xmlMeta) {
                ...
                if (type === 'movie') {
                    ...
                    if (mediaNode) {
                        ...
                        if (partNode) {
                            ...
                            if (libraryKey) {
                                const sectionsUrl = `${baseUri}/library/sections?X-Plex-Token=${accessToken}`;
                                getXml(sectionsUrl, function(sectionsXml) {
                                    ...
                                    // Construir URL con libraryKey y libraryTitle
                                    const redirectUrl = `https://plex-redirect-production.up.railway.app/movie?accessToken=${encodeURIComponent(accessToken)}&baseURI=${encodeURIComponent(baseUri)}&downloadURL=${encodeURIComponent(downloadUrl)}&partKey=${encodeURIComponent(keyBase)}&title=${encodeURIComponent(title)}&year=${year}&posterUrl=${encodeURIComponent(posterUrl)}&tmdbId=${tmdbId}&fileName=${encodeURIComponent(fileName)}&fileSize=${encodeURIComponent(fileSize)}&libraryKey=${encodeURIComponent(libraryKey)}&libraryTitle=${encodeURIComponent(libraryTitle)}`;
                                    console.log('PlexDL: URL construida:', redirectUrl);
                                    window.open(redirectUrl, '_blank');
                                });
                            } else {
                                // Si no hay libraryKey, abrir sin él
                                const redirectUrl = `https://plex-redirect-production.up.railway.app/movie?accessToken=${encodeURIComponent(accessToken)}&baseURI=${encodeURIComponent(baseUri)}&downloadURL=${encodeURIComponent(downloadUrl)}&partKey=${encodeURIComponent(keyBase)}&title=${encodeURIComponent(title)}&year=${year}&posterUrl=${encodeURIComponent(posterUrl)}&tmdbId=${tmdbId}&fileName=${encodeURIComponent(fileName)}&fileSize=${encodeURIComponent(fileSize)}`;
                ...
                if (type === 'show') {
                    ...
                    if (libraryKey) {
                        ...
                        getXml(sectionsUrl, function(sectionsXml) {
                            ...
                            const seasonsUrl = `${baseUri}/library/metadata/${mediaId}/children?X-Plex-Token=${accessToken}`;
                            getXml(seasonsUrl, function(seasonsXml) {
                                ...
                                const redirectUrl = `https://plex-redirect-production.up.railway.app/series?accessToken=${encodeURIComponent(accessToken)}&baseURI=${encodeURIComponent(baseUri)}&title=${encodeURIComponent(title)}&posterUrl=${encodeURIComponent(posterUrl)}&tmdbId=${tmdbId}&seasons=${encodeURIComponent(JSON.stringify(seasons))}&libraryKey=${encodeURIComponent(libraryKey)}&libraryTitle=${encodeURIComponent(libraryTitle)}`;
                                window.open(redirectUrl, '_blank');
                            });
                        });
                    } else {
                        ...
                        getXml(seasonsUrl, function(seasonsXml) {
                            ...
                            const redirectUrl = `https://plex-redirect-production.up.railway.app/series?accessToken=${encodeURIComponent(accessToken)}&baseURI=${encodeURIComponent(baseUri)}&title=${encodeURIComponent(title)}&posterUrl=${encodeURIComponent(posterUrl)}&tmdbId=${tmdbId}&seasons=${encodeURIComponent(JSON.stringify(seasons))}`;
                            window.open(redirectUrl, '_blank');
                        });
                    }
                } else if (type === 'season') {
                    ...
                    const parentUrl = `${baseUri}/library/metadata/${parentRatingKey}?X-Plex-Token=${accessToken}`;
                    getXml(parentUrl, function(parentXml) {
                        ...
                        const redirectUrl = `https://plex-redirect-production.up.railway.app/list?accessToken=${encodeURIComponent(accessToken)}&baseURI=${encodeURIComponent(baseUri)}&seasonRatingKey=${mediaId}&seasonNumber=${seasonNumber}&seriesTitle=${encodeURIComponent(seriesTitle)}&tmdbId=${tmdbId}&parentRatingKey=${parentRatingKey}`;
                        window.open(redirectUrl, '_blank');
                    });
                }
            } else {
